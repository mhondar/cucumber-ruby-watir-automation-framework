class LoginInvitadoPageOld
    def initialize
        selectors = JSON.parse(File.read("./features/selectors/selectors_old/login_invitado_selectors_old.json"))
        # login
        ...
        @apellido = $driver.element(xpath: selectors["invitado"]["apellido"])
        @rut = $driver.element(xpath: selectors["invitado"]["rut"])
        @email = $driver.element(xpath: selectors["invitado"]["email"])
        @repetiremail = $driver.element(xpath: selectors["invitado"]["repetiremail"])
        @btn_ingresar = $driver.element(xpath: selectors["invitado"]["btn_ingresar"])
    end
    ...
    def ingresar_invitado_medio(medio)
        ...
        $tools.insertar_texto(@repetiremail, datos_persona["email"])
    end
